 ...
 *   -  Added a module definition file and made use of Kai Uwe Rommel's 
 *      IsFileSystemFAT() code in os2.c to allow for HPFS long filenames.
 *   -  ifdef'd out the vbeep code, since it doesn't seem to do anything
 *      under OS/2's ANSI driver (OS/2 1.30.1 XR05015).
 *
 * Revision 3.69b  90/04/24            David Kirschbaum
 * Had to do minor tweaks to reduce/remove Turbo C v2.0 error and 
 * warning messages.  Also enabled BIOS (instead of NANSI) screen 
 * I/O and HELP (for ":help" inline help .. nice!).  [Executable
 * uploaded to Simtel & wuarchive as {...}msdos/editor/stevi69x.zip]
 *
...
char	*Version = "STEVIE - Version 3.70";
