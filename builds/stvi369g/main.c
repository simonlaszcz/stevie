...
bool set_want_col;		/* If set, then update Curswant the next time */
...
bool Changed = 0;		/* Set to 1 if something in the file has been */
...
bool got_int = false;		/* set to true when an interrupt occurs (if possible) */

bool interactive = false;	/* set true when main() is ready to roll */
...
static void chk_mline(register char *s);

int main(int argc, char *argv[])
{
	char *initstr = NULL;		/* init string from the environment */
